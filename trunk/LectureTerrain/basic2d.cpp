...
Basic2D::Basic2D()
{
	...
	// Load a sprite from a png-file
        tileset = clan::Image(canvas, "petittileset.png");
	logo = clan::Image(canvas, "logo.png");
	font = clan::Font("tahoma", 24);
        
        //Mon code
        Map terrain(16,16,16);
        std::cout << terrain.getTileAt(2,2) << std::endl;
        terrain.readTmxFile("map1.tmx");
        std::cout << terrain.getTileAt(2,2) << std::endl;
...
bool Basic2D::update()
{
	...
	canvas.clear(clan::Colorf(0.0f, 0.0f, 0.5f));
	...
	logo.draw(canvas, canvas_size.width - logo.get_width(), canvas_size.height - logo.get_height());
        ...
        //TODO : Foutre un tile
        //tileset.draw(canvas, clan::Rectx);
	...
	canvas.fill_rect(clan::Rectf(240.0f, 140.0f, 440.0f, 340.0f), clan::Colorf::gray);

	// Frame the rectangle with red lines
	canvas.draw_box(240.0f, 140.0f, 440.0f, 340.0f, clan::Colorf(1.0f, 1.0f, 0.0f));
